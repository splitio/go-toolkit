...
func TestSetNonTS_NewNonTS_parameters(t *testing.T) {
	...
	if s.Size() != 4 {
		t.Error("NewThreadUnsafeThread: calling with parameters should create a set with size of four")
	}
}
...
func TestSet_New_parameters(t *testing.T) {
	s := NewThreadSafeSet("string", "another_string", 1, 3.14)
...
func TestSet_IsEqual(t *testing.T) {
	...
	// same size, different content
	a := NewThreadSafeSet("1", "2", "3")
	b := NewThreadSafeSet("4", "5", "6")
